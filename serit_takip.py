...
subprocess.run(["python", "perspective_setup.py"])

# --- Perspektif ---
def get_perspective_transform(img_size):
    w, h = img_size
    src = np.float32([
        [0.44*w, 0.63*h], # Sol üst
        [0.56*w, 0.63*h], # Sağ üst
        [0.15*w, 0.95*h], # Sol alt
        [0.85*w, 0.95*h]  # Sağ alt
    ])
    dst = np.float32([
        [w * 0.25, 0],
        [w * 0.75, 0],
...
class LaneDetector:
    ...
    def process(self, image):
        ...
        if lf is not None and rf is not None and is_fit_valid(lf, rf, ploty):
            ...
            #Geçersiz ise önce yeniden find_lane_pixels deneniyor yinede başarısızsa get_last_valid_fit ile son geçerli fit kullanılıyor ve bad_count artırılıyor ve bad_count MAX_BAD_FITS'e ulaşırsa geçmiş temizleniyor
            if lf is not None and rf is not None and is_fit_valid(lf, rf, ploty):
                self.left_fits.append(lf); self.right_fits.append(rf)
                lf = np.mean(self.left_fits, axis=0)
                rf = np.mean(self.right_fits, axis=0)
...
def process_video(input_path='', output_path=''):
    ...
    try:
        while True:
            ...
            bgr_result = cv2.cvtColor(lane_frame, cv2.COLOR_RGB2BGR)
            out.write(bgr_result)
            cv2.imshow("Serit Takip", bgr_result)
        ...
        cap.release()
        out.release()
        print(f"İşlem tamamlandı. Kayıt: {output_path}")
